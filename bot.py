...
def print_market_table(rows, top_setups):
    # V15.3.2 — русский премиальный CMD-интерфейс.
    TURQ = Fore.CYAN
    TURQ_B = Fore.CYAN + Style.BRIGHT
    ...
    def frame(ch="═"):
        return TURQ_B + "╔" + ch*width + "╗"

    def bottom(ch="═"):
        return TURQ_B + "╚" + ch*width + "╝"

    def row(text, color=TURQ):
        plain = re.sub(r"\x1b\[[0-9;]*m", "", text)
        return color + "║" + plain[:width].ljust(width) + "║" + Style.RESET_ALL

    print(frame())
    print(row(" BITGET V15 PRO  •  МОНИТОРИНГ ФЬЮЧЕРСОВ ", TURQ_B))
    print(row(f" Рынок: LIVE  •  Обновление: {now}  •  Режим: СИГНАЛЫ / PAPER", TURQ))
    print(TURQ_B + "╠" + "═"*width + "╣" + Style.RESET_ALL)
    ...
    print(row(
        f"  СИГНАЛЫ: {len(top_setups):02d}   │   ГОТОВЫ: {ready:02d}   │   "
        f"LONG: {longs:02d}   │   SHORT: {shorts:02d}   │   "
        f"ДЕПОЗИТ: ${CFG.deposit:.2f}   │   ПЛЕЧО: {CFG.leverage}x",
        TURQ_B
    ))
    print(TURQ_B + "╠" + "─"*width + "╣" + Style.RESET_ALL)

    headers = [
        ("МОНЕТА",13),("ЦЕНА",14),("SCORE",7),("НАПРАВЛ.",9),
        ("RSI",7),("ОБЪЁМ",8),("СЕТАП",24),("ДАННЫЕ",9),("СТАТУС",12)
    ]
    header="  "+"  ".join(f"{h:^{w}}" for h,w in headers)
    print(row(header,TURQ_B))
    print(TURQ_B + "╠" + "─"*width + "╣" + Style.RESET_ALL)
    ...
    for r in rows:
        ...
        def cell(v,w,align="<"):
            return f"{str(v):{align}{w}}"

        parts = [
            cell(r.get("symbol","-"),13),
            cell(r.get("price","-"),14,">"),
            cell(score,7,">"),
            cell(side,9,"^"),
            cell(f"{rv:.1f}",7,">"),
            ...
            cell(r.get("setup","-"),24),
            cell(r.get("data","-"),9,"^"),
            cell(r.get("status","-"),12,"^")
        ]

        # Все буквы/рамка бирюзовые, кроме специально заданных акцентов.
        print(
            TURQ+"║  "+parts[0]+"  "+parts[1]+"  "
            +(GREEN if score>=CFG.min_score else (YELLOW if score>=60 else RED))+parts[2]+Style.RESET_ALL+
            TURQ+"  "+
            (GREEN if side=="LONG" else (RED if side=="SHORT" else TURQ))+parts[3]+Style.RESET_ALL+
            TURQ+"  "+
            (RED if rv>=70 or (0<rv<=30) else TURQ)+parts[4]+Style.RESET_ALL+
            TURQ+"  "+
            (GREEN if vv>=1.2 else (YELLOW if vv>=0.9 else TURQ))+parts[5]+Style.RESET_ALL+
            TURQ+"  "+TURQ_B+parts[6]+Style.RESET_ALL+
            TURQ+"  "+
            (GREEN if r.get("data")=="LIVE" else (YELLOW if r.get("data")=="STALE" else RED))+parts[7]+Style.RESET_ALL+
            TURQ+"  "+
            (GREEN if r.get("status") in ("ГОТОВ","READY","LIVE") else
             (RED if r.get("status")=="API ERROR" else YELLOW))+parts[8]+Style.RESET_ALL+
            TURQ+"  ║"
        )

    print(TURQ_B+"╠"+"═"*width+"╣"+Style.RESET_ALL)
    print(row(" ЛУЧШИЕ СЕТАПЫ — самые сильные текущие возможности",TURQ_B))

    if top_setups:
        for i,x in enumerate(top_setups[:5],1):
            side=x.get("side","-")
            color=GREEN if side=="LONG" else (RED if side=="SHORT" else TURQ)
            text=(f"  {i}. {x.get('symbol','-'):12}  {side:5}  "
                   f"Score {int(x.get('score',0)):3}/100  •  "
                   f"{x.get('setup','-')}  •  Вход {format_price(x.get('entry',0))}")
            print(row(text,color))
    else:
        print(row("  Нет сильных сетапов выше порога. Ожидание сигнала...",YELLOW))

    print(TURQ_B+"╠"+"─"*width+"╣"+Style.RESET_ALL)
    print(row(
        f" РЕЖИМ PAPER: ВКЛ  •  Виртуальный баланс: ${CFG.deposit:.2f}  •  "
        "Реальные ордера: ВЫКЛ",
        TURQ_B
    ))
    print(bottom())
