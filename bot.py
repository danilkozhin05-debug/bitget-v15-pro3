...
class Bitget:
    ...
    async def place_market_order(self, session, symbol, side, size):
        # IMPORTANT: open the market position WITHOUT a preset SL.
        # The old implementation sent a signal-time SL with the entry order.
        # If price moved between signal creation and fill, Bitget could reject
        # the whole order with HTTP 400 (especially for SHORT).
    ...
    async def place_position_sl(self, session, symbol, side, sl_price, price_place):
        """Attach an exchange-side SL to the already-filled position.

        Bitget's /place-pos-tpsl endpoint creates a position-level SL.  The
        position must already exist, so this is deliberately called only after
        the entry order is confirmed filled and the actual average fill price
        is known. Market execution is requested with executePrice=0.
        """
        hold_side = "long" if side == "LONG" else "short"
        decimals = max(0, int(price_place))
        sl_text = f"{float(sl_price):.{decimals}f}"
        client_oid = "V15SL-" + str(int(time.time() * 1000))
        body = {
            "marginCoin": self.cfg.margin_coin,
            "productType": self.cfg.product_type,
            "symbol": symbol,
            "stopLossTriggerPrice": sl_text,
            "stopLossTriggerType": self.cfg.tp_trigger_type,
            "stopLossExecutePrice": "0",
            "holdSide": hold_side,
            "stopLossClientOid": client_oid,
        }
        return await self._private_request(session, "POST", "/api/v2/mix/order/place-pos-tpsl", body=body)
...
def print_market_table(rows, top_setups):
    # V15 PRO COLOR UI:
    # - Entire table text is turquoise.
    # - LONG is green.
    # - SHORT is red.
    # - Score/status/RSI/volume use additional accent colors.
    # - Fixed-width ASCII layout is preserved for Windows CMD.
    TURQ = Fore.CYAN
    TURQ_BRIGHT = Fore.CYAN + Style.BRIGHT

    print("\033[2J\033[H", end="")
    now = pd.Timestamp.now(tz="UTC").strftime("%Y-%m-%d %H:%M:%S UTC")

    W=[2,10,15,5,8,8,6,7,10,7,6,6,9]
    H=["#","SYMBOL","PRICE","SCORE","POWER","SIDE","RSI","VOL","SETUP","WINDOW","AGE","DATA","STATUS"]
    total=sum(w+2 for w in W)+len(W)-1

    def rule(ch="-"):
        return "+"+"+".join(ch*(w+2) for w in W)+"+"

    def c(v,w,a="left"):
        return " "+fit(v,w,a)+" "

    # Header: turquoise throughout.
    print(TURQ_BRIGHT+"+"+"="*total+"+")
    print(TURQ_BRIGHT+"|"+fit(
        " BITGET STRICT V15 PRO | REAL DATA | 20 FUTURES | LIVE 1s MONITOR ",
        total,"center")+"|")
    print(TURQ+"|"+fit(
        f" Updated: {now} | ticker refresh: 1s | analysis: {CFG.scan_interval_sec}s | candles: 1m/5m/15m ",
        total)+"|")
    print(TURQ+"|"+fit(
        " LIVE=real data | READY=threshold | WAIT=no setup | API ERROR=Bitget unavailable ",
        total)+"|")
    print(TURQ+"+"+"="*total+"+")
    print(TURQ+rule("-"))

    # Header letters remain turquoise as requested.
    print(TURQ+"|"+"|".join(c(paint(h, TURQ_BRIGHT, True),w,"center")
                              for h,w in zip(H,W))+"|")
    print(TURQ+rule("-"))
    ...
    for i,r in enumerate(rows,1):
        ...
        side=r.get("side","-")
        vals=[
            str(i), r.get("symbol","-"), r.get("price","-"), str(score),
            progress(score,8), side, f"{r.get('rsi',0):.1f}",
            f"{r.get('vol',0):.1f}x", r.get("setup","-"),
            r.get("window","-"), r.get("age","-"),
            r.get("data","-"), r.get("status","-")
        ]

        out=[]
        for j,(v,w) in enumerate(zip(vals,W)):
            # Default: every table character turquoise.
            col=TURQ
            ...
            if j == 0:
                ...
                t=paint(c(v,w),TURQ_BRIGHT,True)
            elif j == 2:
                # Price: turquoise, bright for readability.
                t=paint(c(v,w,"right"),TURQ_BRIGHT,True)
            elif j == 3:
                # Score: keep turquoise theme, with useful accent by strength.
                if score >= CFG.min_score:
                    ...
                    col=Fore.RED
                t=paint(c(v,w,"right"),col,True)
            elif j == 4:
                # Power bar remains turquoise.
                t=paint(c(v,w,"center"),TURQ_BRIGHT,True)
            elif j == 5:
                # REQUIRED: LONG green, SHORT red.
                col=Fore.GREEN if side=="LONG" else (
                    Fore.RED if side=="SHORT" else TURQ)
                t=paint(c(v,w,"center"),col,True)
            elif j == 6:
                rv=float(v)
                col=Fore.RED if rv>=70 or (rv>0 and rv<=30) else TURQ
                t=paint(c(v,w,"right"),col)
            elif j == 7:
                vr=float(v[:-1]) if str(v).endswith("x") else 0
                col=Fore.GREEN if vr>=1.2 else (
                    Fore.YELLOW if vr>=0.9 else TURQ)
                t=paint(c(v,w,"right"),col)
            elif j == 8:
                # Setup stays turquoise so the table remains visually coherent.
                t=paint(c(v,w,"center"),TURQ_BRIGHT,True)
            elif j == 9:
                t=paint(c(v,w,"center"),TURQ,True)
            elif j == 10:
                t=paint(c(v,w,"right"),TURQ)
            elif j == 11:
                # LIVE green, STALE yellow, unavailable red.
                col=Fore.GREEN if v=="LIVE" else (
                    Fore.YELLOW if v=="STALE" else Fore.RED)
                t=paint(c(v,w,"center"),col,True)
            else:
                label={"ГОТОВ":"READY","LIVE":"LIVE",
                       "API ERROR":"API ERROR","WAIT":"WAIT"}.get(v,v)
                col=Fore.GREEN if v in ("ГОТОВ","LIVE") else (
                    Fore.RED if v=="API ERROR" else (
                        Fore.YELLOW if v=="WAIT" else TURQ))
                t=Back.BLACK+col+Style.BRIGHT+c(label,w,"center")+Style.RESET_ALL

            out.append(t)

        # Grid itself is turquoise; colored cell text overrides only where intended.
        print(TURQ+"|"+"|".join(out)+"|")

    print(TURQ+rule("-"))
    print(TURQ_BRIGHT+Style.BRIGHT+
          f" DEPOSIT ${CFG.deposit:.2f} | LEVERAGE {CFG.leverage}x | "
          f"RISK {CFG.risk_pct:.2f}% | THRESHOLD {CFG.min_score}/100 | UI REFRESH 1s")

    print(TURQ_BRIGHT+Style.BRIGHT+
          f" TOP SETUPS ({min(3,len(top_setups))}):")

    if top_setups:
        for i,s in enumerate(top_setups[:3],1):
            # REQUIRED: LONG green, SHORT red.
            col=Fore.GREEN if s["side"]=="LONG" else (
                Fore.RED if s["side"]=="SHORT" else TURQ)
            print("  "+paint(
                f"{i}. {s['symbol']} {s['side']} | {s['setup']} | "
                f"Score {s['score']}/100 | Entry {format_price(s['entry'])}",
                col,True))
    else:
        print(TURQ+"  Пока нет сильных сетапов выше порога.")

    print(TURQ+
          " Score — сила совпадения условий, не гарантия прибыли. "
          "LIVE_TRADING=false: ордера не отправляются.")
                        ...
                        try:
                            ...
                            if len(active)<CFG.max_auto_positions and sym not in active:
                                ...
                                if max_available > 0 and total_required <= max_available:
                                    result=await bg.place_market_order(session,sym,sig["side"],state["qty"])
                                    ...
                                    # Rebuild the SL from the ACTUAL fill price instead of
                                    # the stale signal price.  This preserves the original
                                    # risk distance as a percentage while guaranteeing that
                                    # a LONG SL is below entry and a SHORT SL is above entry.
                                    ep=float(ep)
                                    signal_entry=float(sig["entry"])
                                    signal_sl=float(sig["sl"])
                                    risk_pct=abs(signal_entry-signal_sl)/signal_entry if signal_entry else 0.0
                                    if risk_pct <= 0 or not np.isfinite(risk_pct):
                                        raise RuntimeError("Invalid SL distance from signal")
                                    actual_sl = ep*(1-risk_pct) if sig["side"]=="LONG" else ep*(1+risk_pct)
                                    price_place=int(state["contract"].get("pricePlace",4) or 4)
                                    actual_sl=float(f"{actual_sl:.{max(0,price_place)}f}")

                                    # Final direction check against the latest ticker.
                                    # If price has already crossed the rebuilt stop, do not
                                    # submit an invalid SL; close the fresh position instead.
                                    live_px=float(price)
                                    if (sig["side"]=="LONG" and actual_sl >= live_px) or (sig["side"]=="SHORT" and actual_sl <= live_px):
                                        try:
                                            await bg.close_partial_market(session,sym,sig["side"],actual_qty)
                                        except Exception as ce:
                                            logging.error("SL PRECHECK CLOSE %s: %s",sym,ce)
                                        raise RuntimeError(f"SL became invalid after fill: side={sig['side']} fill={ep} live={live_px} sl={actual_sl}")

                                    # Put the SL on Bitget only after the position exists.
                                    # This removes the old HTTP 400 failure mode caused by
                                    # sending a stale signal-time SL with the entry order.
                                    try:
                                        sl_result=await bg.place_position_sl(session,sym,sig["side"],actual_sl,price_place)
                                        if not isinstance(sl_result,dict):
                                            raise RuntimeError(f"Bitget returned invalid SL response: {sl_result}")
                                    except Exception as sl_error:
                                        # Never leave a newly opened position unprotected.
                                        try:
                                            await bg.close_partial_market(session,sym,sig["side"],actual_qty)
                                        except Exception as ce:
                                            logging.error("SL ROLLBACK CLOSE %s: %s",sym,ce)
                                        raise RuntimeError(f"Bitget SL was not accepted: {sl_error}. Position close was attempted.")
                                    levels=[]
                                    ...
                                    try:
                                        ...
                                        # The position-level SL is already live on Bitget; if
                                        # the TP ladder fails, cancel accepted TPs and attempt
                                        # an emergency close rather than leaving a half-managed
                                        # position.
                                        try:
                                    ...
                                    await notify(
                                        ...
                                        f"SL: {actual_sl:.10g} — exchange-side position SL\nEntry Order ID: {oid}\n"
                                        ...
                                        "high")
                                    csv_log({"time":pd.Timestamp.utcnow().isoformat(),"symbol":sym,"side":sig["side"],"setup":sig["setup"],"score":current_score,"entry":current_price,"sl":actual_sl,"tp":sig["tp"],"qty":actual_qty,"action":"AUTO_ORDER","order_id":oid})
                                else:
                                    await notify(
