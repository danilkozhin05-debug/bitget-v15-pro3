...
def print_market_table(rows, top_setups):
    # Strict fixed-width ASCII grid: no Unicode-width drift can move columns.
    print("\033[2J\033[H", end="")
    now = pd.Timestamp.now(tz="UTC").strftime("%Y-%m-%d %H:%M:%S UTC")
    # Fixed ASCII-safe widths. No box-drawing characters inside cells, so columns stay aligned in Windows CMD.
    ...
    def rule(ch="-"): return "+"+"+".join(ch*(w+2) for w in W)+"+"
    def c(v,w,a="left"): return " "+fit(v,w,a)+" "
    print(Fore.CYAN+Style.BRIGHT+"+"+"="*total+"+")
    print(Fore.CYAN+Style.BRIGHT+"|"+fit(" BITGET STRICT V15 PRO | REAL DATA | 20 FUTURES | LIVE 1s MONITOR ",total,"center")+"|")
    print(Fore.CYAN+"|"+fit(f" Updated: {now} | ticker refresh: 1s | analysis: {CFG.scan_interval_sec}s | candles: 1m/5m/15m ",total)+"|")
    print(Fore.CYAN+"|"+fit(" LIVE=real data | READY=threshold | WAIT=no setup | API ERROR=Bitget unavailable ",total)+"|")
    print(Fore.CYAN+"+"+"="*total+"+")
    print(Fore.CYAN+rule("-"))
    print(Fore.CYAN+"|"+"|".join(c(paint(h,Fore.YELLOW,True),w,"center") for h,w in zip(H,W))+"|")
    print(Fore.CYAN+rule("-"))
    for i,r in enumerate(rows,1):
        score=int(r.get("score",0)); side=r.get("side","-")
        vals=[str(i),r.get("symbol","-"),r.get("price","-"),str(score),progress(score,8),side,f"{r.get('rsi',0):.1f}",f"{r.get('vol',0):.1f}x",r.get("setup","-"),r.get("window","-"),r.get("age","-"),r.get("data","-"),r.get("status","-")]
        out=[]
        for j,(v,w) in enumerate(zip(vals,W)):
            if j==0: t=paint(c(v,w),Fore.LIGHTBLACK_EX)
            elif j==1: t=paint(c(v,w),Fore.CYAN,True)
            elif j==2: t=paint(c(v,w,"right"),Fore.WHITE)
            elif j==3:
                col=Fore.GREEN if score>=CFG.min_score else (Fore.YELLOW if score>=60 else Fore.RED); t=paint(c(v,w,"right"),col,True)
            elif j==4: t=c(v,w,"center")
            elif j==5:
                col=Fore.GREEN if side=="LONG" else (Fore.RED if side=="SHORT" else Fore.LIGHTBLACK_EX); t=paint(c(v,w,"center"),col,True)
            elif j==6:
                rv=float(v); col=Fore.RED if rv>=70 or (rv>0 and rv<=30) else Fore.GREEN; t=paint(c(v,w,"right"),col)
            elif j==7:
                vr=float(v[:-1]) if str(v).endswith("x") else 0; col=Fore.GREEN if vr>=1.2 else Fore.YELLOW; t=paint(c(v,w,"right"),col)
            elif j==8: t=paint(c(v,w,"center"),Fore.MAGENTA,True)
            elif j==9: t=paint(c(v,w,"center"),Fore.CYAN,True)
            elif j==10: t=paint(c(v,w,"right"),Fore.WHITE)
            elif j==11:
                col=Fore.GREEN if v=="LIVE" else (Fore.YELLOW if v=="STALE" else Fore.RED); t=paint(c(v,w,"center"),col,True)
            else:
                label={"ГОТОВ":"READY","LIVE":"LIVE","API ERROR":"API ERROR","WAIT":"WAIT"}.get(v,v)
                col=Fore.GREEN if v in ("ГОТОВ","LIVE") else (Fore.RED if v=="API ERROR" else Fore.YELLOW)
                t=Back.BLACK+col+Style.BRIGHT+c(label,w,"center")+Style.RESET_ALL
            out.append(t)
        print(Fore.CYAN+"|"+"|".join(out)+"|")
    print(Fore.CYAN+rule("-"))
    print(Fore.MAGENTA+Style.BRIGHT+f" DEPOSIT ${CFG.deposit:.2f} | LEVERAGE {CFG.leverage}x | RISK {CFG.risk_pct:.2f}% | THRESHOLD {CFG.min_score}/100 | UI REFRESH 1s")
    print(Fore.GREEN+Style.BRIGHT+f" TOP SETUPS ({min(3,len(top_setups))}):")
    if top_setups:
        for i,s in enumerate(top_setups[:3],1):
            col=Fore.GREEN if s["side"]=="LONG" else Fore.RED
            print("  "+paint(f"{i}. {s['symbol']} {s['side']} | {s['setup']} | Score {s['score']}/100 | Entry {format_price(s['entry'])}",col,True))
    else:
        print(Fore.YELLOW+"  Пока нет сильных сетапов выше порога.")
    print(Fore.LIGHTBLACK_EX+" Score — сила совпадения условий, не гарантия прибыли. LIVE_TRADING=false: ордера не отправляются.")
